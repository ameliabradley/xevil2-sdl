...
void Utils::seed_random(int seed) {
#ifdef USE_RANDOM
  srandom(seed);
#else
...
char* Utils::get_OS_info() {
  ...
  // FIXME: Disabled OS detection due to deprecation of GetVersionEx
  /*
...
void DebugInfo::initialize() {
  ...
  if (_on) {
    ...
    outStream.open("c:\\out.txt",std::ios::out);
    outStream.close();
  }
#endif
}
...
#if WIN32
  std::fstream outStream;
  outStream.open("c:\\out.txt",std::ios::app); 
