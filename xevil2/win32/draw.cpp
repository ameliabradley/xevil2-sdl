...
static void World_init_x_helper(Xvars& xvars,BitmapSpec* specs,int bitsNum,
  ...
  for (int n = 0; n < bitsNum; n++) {
    ...
    if (!xvars.load_surface(surf,CPoint(surfPos.x,surfPos.y),specs[n].id)) {
      std::ostrstream str;
      str << "World::init_x: Couldn't load " << errorMsg << std::ends;
      coutRESULT(str.str());
      delete str.str();
      return;
    }    
...
Boolean Locator::generate_xpm(Xvars& xvars,const char* genDir,
  ...
  for (TickType tt = 0; tt < TICK_MAX; tt++) {
    for (int hp = 0; hp < CO_DIR_HALF_PURE; hp++) {
      ...
      std::ostrstream varName;
      varName << "tick_" << tt << '_' << hp << std::ends;
      ...
      if (!val) {
        std::ostrstream err;
        err << "Failed to write tick tt=" << tt << " hp=" << hp << " fullName=" << fullName << std::ends;
        coutRESULT(err.str());
        delete err.str();
        return False;
      }
