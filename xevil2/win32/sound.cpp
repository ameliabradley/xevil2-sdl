...
SoundManager::stopSound(SoundName p_name) {
  ...
  HRESULT     dsrval;
	int t_index;
	int i;
 	for (i=0;i<m_soundbuffers.GetSize();i++) {
...
SoundManager::DSLoadSoundBuffer(IDirectSound *pDS, LPCTSTR lpName)
{
    ...
    {
        dsBD.dwSize = sizeof(dsBD);
		dsBD.dwFlags = DSBCAPS_STATIC; // FIXME: DSBCAPS_STATIC | DSBCAPS_CTRLDEFAULT;
