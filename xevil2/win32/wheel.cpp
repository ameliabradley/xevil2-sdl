...
void Wheel::init_x(LocatorP l,Xvars &xvars,IXCommand command,void*) {
  ...
  for (int i = 0; i < numbitmaps; i++) {
    ...
    if (!xvars.load_surface(itemSurf,CPoint(0,0),bitmapids[i])) {
      std::ostrstream dbg;
      dbg << "load_ssurface fails on " << i << " which has ID " << bitmapids[i] << std::ends;
      coutRESULT(dbg.str());
      delete dbg.str();
      DHRESULT(DDERR_GENERIC,return);
    }
...
Wheel::setIndex(int p_item) {
  if (p_item < 0 || p_item >= numbitmaps) {
    ASSERT(0);
    std::ostrstream dbg;
    dbg << "Wheel::setIndex invalid index " << p_item << std::ends;
    coutRESULT(dbg.str());
    delete dbg.str();
    m_item = 0;
  }
...
Wheel::setIndexByClassId(ClassId p_cid) {
  if (!(p_cid >= 0 && p_cid <= A_None)) {
    std::ostrstream dbg;
    dbg << "Wheel::setIndexByClassId, invalid cid " << p_cid << std::ends;
    coutRESULT(dbg.str());
    delete dbg.str();
...
Wheel::setReadyAtIndex(Wheel::ReadyState p_state,int p_index) {
  if (p_index < numbitmaps && p_index > -1) {
    ...
    dbg << "setReadyAtIndex to " << p_index 
        << " when max value is " << numbitmaps << std::ends;
    coutRESULT(dbg.str());
    delete dbg.str();
    ASSERT(0);
  }
